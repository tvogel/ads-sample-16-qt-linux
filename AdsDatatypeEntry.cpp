...
QString adsSymbolFlagsToString(uint32_t flags) {
  QStringList flagStrings;
  if (flags & ADSSYMBOLFLAG_PERSISTENT) flagStrings << "PERSISTENT";
  if (flags & ADSSYMBOLFLAG_BITVALUE) flagStrings << "BITVALUE";
  if (flags & ADSSYMBOLFLAG_REFERENCETO) flagStrings << "REFERENCETO";
  if (flags & ADSSYMBOLFLAG_TYPEGUID) flagStrings << "TYPEGUID";
  if (flags & ADSSYMBOLFLAG_TCCOMIFACEPTR) flagStrings << "TCCOMIFACEPTR";
  if (flags & ADSSYMBOLFLAG_READONLY) flagStrings << "READONLY";
  if (flags & ADSSYMBOLFLAG_CONTEXTMASK) flagStrings << "CONTEXTMASK";
  return flagStrings.join(" | ");
}

QString adsDatatypeFlagsToString(uint32_t flags) {
  QStringList flagStrings;
  if (flags & ADSDATATYPEFLAG_DATATYPE) flagStrings << "DATATYPE";
  if (flags & ADSDATATYPEFLAG_DATAITEM) flagStrings << "DATAITEM";
  if (flags & ADSDATATYPEFLAG_REFERENCETO) flagStrings << "REFERENCETO";
  if (flags & ADSDATATYPEFLAG_METHODDEREF) flagStrings << "METHODDEREF";
  if (flags & ADSDATATYPEFLAG_OVERSAMPLE) flagStrings << "OVERSAMPLE";
  if (flags & ADSDATATYPEFLAG_BITVALUES) flagStrings << "BITVALUES";
  if (flags & ADSDATATYPEFLAG_PROPITEM) flagStrings << "PROPITEM";
  if (flags & ADSDATATYPEFLAG_TYPEGUID) flagStrings << "TYPEGUID";
  if (flags & ADSDATATYPEFLAG_PERSISTENT) flagStrings << "PERSISTENT";
  if (flags & ADSDATATYPEFLAG_COPYMASK) flagStrings << "COPYMASK";
  if (flags & ADSDATATYPEFLAG_TCCOMIFACEPTR) flagStrings << "TCCOMIFACEPTR";
  if (flags & ADSDATATYPEFLAG_METHODINFOS) flagStrings << "METHODINFOS";
  if (flags & ADSDATATYPEFLAG_ATTRIBUTES) flagStrings << "ATTRIBUTES";
  if (flags & ADSDATATYPEFLAG_ENUMINFOS) flagStrings << "ENUMINFOS";
  if (flags & ADSDATATYPEFLAG_ALIGNED) flagStrings << "ALIGNED";
  if (flags & ADSDATATYPEFLAG_STATIC) flagStrings << "STATIC";
  if (flags & ADSDATATYPEFLAG_SPLEVELS) flagStrings << "SPLEVELS";
  if (flags & ADSDATATYPEFLAG_IGNOREPERSIST) flagStrings << "IGNOREPERSIST";
  if (flags & ADSDATATYPEFLAG_ANYSIZEARRAY) flagStrings << "ANYSIZEARRAY";
  if (flags & ADSDATATYPEFLAG_PERSIST_DT) flagStrings << "PERSIST_DT";
  if (flags & ADSDATATYPEFLAG_INITONRESET) flagStrings << "INITONRESET";
...
QJsonObject AdsDatatypeEntry::toJson() const
{
  ...
  if (arrayDim > 0) {
    ...
    for (int i = 0; i < arrayDim; ++i) {
      const AdsDatatypeArrayInfo *info = ArrayInfo() + i;
